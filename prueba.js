function probarParseoQR(stringEscaneado) {
    try {
        // Parseo directo del JSON
        if (stringEscaneado.startsWith('{') && stringEscaneado.endsWith('}')) {
            const config = JSON.parse(stringEscaneado);

            console.log('✅ Objeto config cargado correctamente:');
...
// === PRUEBA ===
const stringPrueba = '{"ip":"192.168.1.40","puerto":2000,"apiKey":"e4ec4963-e526-4e02-8007-5f86e804b6ff"}';

const resultado = probarParseoQR(stringPrueba);
...
app.post('/api/escanear', (req, res) => {
    try {
        ...
        setImmediate(async () => {
            try {
                await keyboard.type(codigoLimpio);
